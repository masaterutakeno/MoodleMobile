...
function add_to_log($courseid, $module, $action, $url='', $info='', $cm=0, $user=0) {
    ...
    // routine basis.
    if (textlib::strlen($action) > 40) {
        $action = textlib::substr($action, 0, 37) . '...';
...
/**
 * Add an entry to the @iatecam device log table.
 *
 * Add an entry to the @iatecam device log table.  These are "action" focussed rather
 * than web server hits, and provide a way to easily reconstruct what
 * any particular student has been doing.
 *
 * @package core
 ...
 * @param    string  $url     The file and parameters used to see the results of the action
 * @param    string  $info    Additional description information
 * @param    string  $detalhes Additional device information and coordinate
 * @param    string  $cm      The course_module->id if there is one
 * @param    string  $user    If log regards $user other than $USER
 * @return void
 */
function add_to_iat_log($courseid, $module, $action, $url='', $info='', $detalhes, $cm=0, $user=0) {
    // Note that this function intentionally does not follow the normal Moodle DB access idioms.
    // This is for a good reason: it is the most frequently used DB update function,
    // so it has been optimised for speed.
    global $DB, $CFG, $USER;
    ...
    $log = array('time'=>$timenow, 'userid'=>$userid, 'course'=>$courseid, 'ip'=>$REMOTE_ADDR, 'module'=>$module,
                 'cmid'=>$cm, 'action'=>$action, 'url'=>$url, 'info'=>$info, 'detalhes'=>$detalhes);

    try {
        $DB->insert_record_raw('iat_log', $log, false);
    } catch (dml_exception $e) {
        debugging('Error: Could not insert a new entry to the Moodle log. '. $e->error, DEBUG_ALL);
    }
}
